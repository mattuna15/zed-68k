/*  ------------------------------------------------------------------------
Program      : cr (gcc -o cr.exe cr.c)

Description  : Commandline Utility

               Converts "Unix" Text Files to Windows Text Files by adding
			   carriage-returns (CR = 0x0d) to the "Unix" sequential text
			   file's linefeed termination (LF = 0x0a).

Input:  "Unix" Sequential Text File
Output: Windows Sequential Text File
...
Revision     : 1.0 First Release of rmcr
               1.1 First Release of cr (a variation of rmcr)
...
int main(int argc, char *argv[])
{
	...
	FILE *fp2;
	if (argc != 3) {
		fprintf(stderr, "Usage: cr <in_filename> <out_filename>\n");
		return(1);
	}
